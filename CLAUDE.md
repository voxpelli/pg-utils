...
- **`installed-check` errors**: If `npm run check` fails with "Invalid version", run `npm install` to sync dependencies
...
`index.d.ts` is **generated by `npm run build`** but **committed to git** — the `clean` script preserves it. `lib/*.d.ts` files are generated and cleaned between builds. After adding new exports, run `npm run build` and commit `index.d.ts`.

`PgTestHelpersOptions` is re-exported as a type via a `@typedef` comment in `index.js`.
...
- All numeric options (lockId, timeouts) validated with `Number.isSafeInteger` — not `isFinite` or `isInteger`
