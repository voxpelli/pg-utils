...
export class PgTestHelpers {
  ...
  constructor (options) {
    ...
    if (lockId !== undefined && (typeof lockId !== 'number' || !Number.isInteger(lockId))) {
      throw new TypeError('Invalid lockId, expected an integer');
    }
    if (lockTimeoutMs !== undefined && (typeof lockTimeoutMs !== 'number' || !Number.isFinite(lockTimeoutMs) || lockTimeoutMs < 0)) {
      throw new TypeError('Invalid lockTimeoutMs, expected a non-negative finite number');
    }
    if (statementTimeoutMs !== undefined && (typeof statementTimeoutMs !== 'number' || !Number.isFinite(statementTimeoutMs) || statementTimeoutMs < 0)) {
      throw new TypeError('Invalid statementTimeoutMs, expected a non-negative finite number');
    }
    if (idleInTransactionTimeoutMs !== undefined && (typeof idleInTransactionTimeoutMs !== 'number' || !Number.isFinite(idleInTransactionTimeoutMs) || idleInTransactionTimeoutMs < 0)) {
      throw new TypeError('Invalid idleInTransactionTimeoutMs, expected a non-negative finite number');
    }
