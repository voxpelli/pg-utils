...
export class PgTestHelpers {
  ...
  /**
   * Acquire an exclusive database lock to prevent other pools from accessing the database.
   *
   * The lock is held until {@link end} is called.
   *
   * @returns {Promise<void>}
   */
  async #ensureLocked () {
    ...
    const connectionString = this.#pool.options.connectionString;

    if (!connectionString) {
      throw new Error('Cannot lock: no connection string available');
    }

    this.#lockClient = await createAndLockConnection(connectionString);
  }
